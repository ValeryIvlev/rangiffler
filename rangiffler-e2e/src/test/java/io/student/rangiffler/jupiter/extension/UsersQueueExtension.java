...
public class UsersQueueExtension implements
    ...
    static {
        EMPTY_USERS.add(new StaticUser("bee", STANDART_PASSWORD, null, null, null));
        WITH_FRIEND_USERS.add(new StaticUser("duck", STANDART_PASSWORD, "beer", null, null));
        WITH_INCOME_REQUEST_USERS.add(new StaticUser("dima", STANDART_PASSWORD, null, "bee", null));
        WITH_OUTCOME_REQUEST_USERS.add(new StaticUser("barsik", STANDART_PASSWORD, null, null, "duck"));
    }
    ...
    private void returnUser(UserType.Type type, StaticUser user) {
        switch (type) {
        ...
        Arrays.stream(context.getRequiredTestMethod().getParameters())
                ...
                .forEach(ut -> {
                    ...
                    while (user.isEmpty() && sw.getTime(TimeUnit.SECONDS) < 30) {
                        user = Optional.ofNullable(pollUser(ut.value()));
                    }
                    ...
                    user.ifPresentOrElse(
                            u -> ((Map<UserType, StaticUser>)
                                    context.getStore(NAMESPACE)
                                    .getOrComputeIfAbsent(context.getUniqueId(),
                                            key -> new HashMap<>())).put(ut, u)
                            ,
                            () -> {
    ...
    public void afterEach(ExtensionContext context) {
        ...
        for (Map.Entry<UserType, StaticUser> e : map.entrySet()) {
            returnUser(e.getKey().value(), e.getValue());
        }
